...
class pdf {
	static public function convert_pdf_to_image($pdf_path, $image_path, $options = array() ) {
		...
		*/
		...
		exec($options['ghostscript_path'] .' -q -dQUIET -dSAFER -dBATCH -dNOPAUSE -dNOPROMPT -dMaxBitmap=500000000 -dAlignToPixels=0 -dGridFitTT=2 -sDEVICE='. $sdevice .' -dTextAlphaBits=4 -dGraphicsAlphaBits=4 -r'. $options['resolution'] .' -dJPEGQ='. $options['jpg_quality'] .' -sOutputFile='. $image_path .' '. $pdf_path .' 2>&1', $coutput, $returncode);
		if (!empty($coutput)) {
