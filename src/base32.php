 ...
 *
 * For encoding a 32-character set, such as eg. the 26 upper-case letters A–Z and the digits 2–7
 *
 * Check also:
 * - base62 for converting integers only: https://stackoverflow.com/questions/4964197/converting-a-number-base-10-to-base-62-a-za-z0-9
...
class base32 {
	 ...
	 *
	 * @param string $string : Plain text or binary string
	 * @param string $char_map : The character map to use for the output. Provide your own map of 32 unique characters, or use one of these standards:
	 *   - `csRFC3548` : the standard base32 character map (default)
	 *   - `csSafe` : standard except it excludes I, L and U
	 *   - `csExtraSafe` : standard except it excludes 0, 1, I and O
	 *   - `cs09AV` : compatible with MIME::Base32
	 * @return string : Base32 encoded string
	 */
	public static function encode($string, $char_map = false) {
		$base32 = new Base32Internal();
		if ($char_map && in_array($char_map, ['csRFC3548', 'csSafe', 'csExtraSafe', 'cs09AV'])) {
			$base32->setCharset(constant('winternet\jensenfw2\Base32Internal::'.$char_map));
		} elseif ($char_map && strlen($char_map) === 32) {
			$base32->setCharset($char_map);
		} elseif ($char_map) {
			core::system_error('Invalid character map for base32 encoding.', ['Set' => $char_map]);
		}
		return $base32->fromString($string);
	}

	/**
	 * Decode a base32 string
	 *
	 * @param string $string : Base32 string to decode
	 * @param string $char_map : The character map to use for the output. Provide your own map of 32 unique characters, or use one of these standards:
	 *   - `csRFC3548` : the standard base32 character map (default)
	 *   - `csSafe` : standard except it excludes I, L and U
	 *   - `csExtraSafe` : standard except it excludes 0, 1, I and O
	 *   - `cs09AV` : compatible with MIME::Base32
	 * @return string : Plain text or binary
	 */
	public static function decode($string, $char_map = false) {
		$base32 = new Base32Internal();
		if ($char_map && in_array($char_map, ['csRFC3548', 'csSafe', 'csExtraSafe', 'cs09AV'])) {
			$base32->setCharset(constant('winternet\jensenfw2\Base32Internal::'.$char_map));
		} elseif ($char_map && strlen($char_map) === 32) {
		...
		if (!preg_match('|^['. preg_quote($base32->_charset) .']+$|', $string)) {
			core::system_error('Invalid input string for base32 decoding.', ['String' => $string]);
		}
		return $base32->toString($string);
	}

}

////////////////////////////////////////////////////////////////////////////////
//                CLASS BELOW IS USED BY THE ABOVE TWO FUNCTIONS              //
////////////////////////////////////////////////////////////////////////////////
 ...
 * Notes
 * -----------
 * For dealing with humans it's best to use csSafe or csExtraSafe rather then csRFC3548
 *
 */
...
class Base32Internal {
	 ...
	 * csExtraSafe
	 *
	 * This character set is designed to be even more human friendly
	 * It's easy to read, write, pronounce and interpret
	 * 0, 1, I, O is not used
	 * Made by Allan Jensen, WinterNet Studio
	 *
	 */
	const csExtraSafe = '23456789ABCDEFGHJKLMNPQRSTUVWXYZ';
	 ...
	 * @var string
	 */
	// CUSTOMIZATION: changed from "protected" to "var" because I need to get it above
	var $_charset;
