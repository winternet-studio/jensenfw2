 ...
 * Paperg add the text nad plaintext to the selectors for the find syntax.  plaintext impliex text in the innertext of a node.  text implies that the tag is a text node.
...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
    ...
    function convert_text($text) {
        ...
        if ($this->dom) {
            ...
            $targetCharset = strtoupper($this->dom->target_charset);
        }
...
class simple_html_dom {
    ...
    protected function parse_charset()
    {
        ...
        // Set the charset to UTF-8 by default if we couldn't find anything else
        if (empty($charset))
        {
            $charset = 'UTF-8';
        }
    ...
    function __get($name) {
        switch($name) {
            ...
            case 'target_charset': return $this->target_charset;
        }
    }
    ...
    static public function file_get_html($url, $use_include_path = false, $context=null, $offset = 0, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = 'UTF-8', $stripRN=true, $defaultBRText="\r\n") {
